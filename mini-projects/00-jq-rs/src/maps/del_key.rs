...
mod tests {
    ...
    #[test]
    fn test_basic_del() {
        let del_map = DelMapKey { key: "a".to_string() };
        let values = del_map.map(Ok(vec![serde_json::from_str("{\"a\":1,\"b\":2}").unwrap()])).unwrap();
    ...
    fn test_del_nonexistent_key() {
        let del_map = DelMapKey { key: "f".to_string() };
        let values = del_map.map(Ok(vec![serde_json::from_str("{\"a\":1}").unwrap()])).unwrap();
        assert_eq!(values.len(), 1);
