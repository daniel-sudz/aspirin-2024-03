...
impl BackgroundMultiDevice {
    ...
    pub fn from_auto_configure(num_devices: usize) -> Result<Self> {
        ...
        let mut background_multidevice = BackgroundMultiDevice {
            state,
            pos,
            controller_input,
            enable,
...
impl MultiDevice {
    pub fn from_auto_configure(num_devices: usize) -> Result<Self> {
        ...
        let commanders = match ports.len().cmp(&num_devices) {
            ...
            std::cmp::Ordering::Greater => Err(anyhow::anyhow!(
                "Too many devices found - need {}, found {}",
                num_devices,
                ports.len()
            )),
        }?;
                ...
                if let Some(input_value) = input {
                    match input_value {
                        ControllerInput::StopGame => {
                            Commander::run_on_all_commanders(
                                &mut self.commanders,
                                |commander: &mut Commander| commander.transition_to_complete(),
                            )?;
                    ...
                    match input_value {
                        ControllerInput::ResetGame => {
                            Commander::run_on_all_commanders(
                                &mut self.commanders,
                                |commander: &mut Commander| {
                                    commander.transition_to_pending_init_from_complete()
                                },
                            )?;
                            DeviceState::PendingInit
                        }
                        ControllerInput::RestartGame => {
                            Commander::run_on_all_commanders(
                                &mut self.commanders,
                                |commander: &mut Commander| {
                                    commander.transition_to_pending_start_from_complete()
                                },
                            )?;
                            DeviceState::PendingStart
                        }
                        ControllerInput::StartController => {
                            Commander::run_on_all_commanders(
                                &mut self.commanders,
                                |commander: &mut Commander| {
                                    commander.transition_to_running_from_complete()
                                },
                            )?;
                            DeviceState::Running
                        }
    ...
    pub fn get_pos(&self) -> Vec<(i32, i32)> {
        self.commanders
            .iter()
            .map(|commander| commander.get_pos())
            .collect()
    }
